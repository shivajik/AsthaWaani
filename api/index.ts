...
app.get('/sitemap.xml', async (req: Request, res: Response) => {
  try {
    ...
    for (const post of publishedPosts) {
      ...
      xml += `    <xhtml:link rel="alternate" hreflang="x-default" href="${baseUrl}/blog/${post.slug}" />\n`;
      xml += `  </url>\n`;
    }
    ...
    res.status(503).set('Retry-After', '60').send('Service temporarily unavailable');
  }
});
...
// Catch-all route for page requests (handles crawler prerendering for social sharing)
...
<body></body>
  ...
  try {
    ...
    let ogImage = `${baseUrl}/opengraph.jpg`;
    let ogType = 'website';
    ...
    if (blogMatch) {
      ...
      if (post && post.status === 'published') {
        ...
        contentHtml = `<h1>${post.title}</h1><p>${post.excerpt || ''}</p>`;
        
        if (post.publishedAt) {
          articleMeta = `<meta property="article:published_time" content="${new Date(post.publishedAt).toISOString()}" />`;
        }
        if (post.updatedAt) {
