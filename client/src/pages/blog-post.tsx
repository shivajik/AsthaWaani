          ...
          <main className="md:col-span-3">
            ...
            <div className="mt-12 rounded-xl bg-[hsl(45,80%,92%)] dark:bg-[hsl(45,40%,15%)] py-10 px-6 text-center space-y-5">
              ...
              <div className="flex items-center justify-center gap-4 flex-wrap">
                <a
                  href="https://www.youtube.com/@asthawaani?sub_confirmation=1"
                  target="_blank"
                ...
                >
                  ...
                  {language === "hi" ? "YouTube पर सब्सक्राइब करें" : "Subscribe on YouTube"}
                </a>
                <Link href="/contact">
                  <Button variant="outline" className="rounded-full px-5 py-2.5 font-semibold">
              ...
              <div className="mt-12 pt-8 border-t border-border">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                  {relatedPosts.map((rp: Post) => {
                    const rpTitle = isHindi ? rp.titleHi || rp.title : rp.title;
                    const rpExcerpt = isHindi ? rp.excerptHi || rp.excerpt : rp.excerpt;
                    return (
                      <Link key={rp.id} href={`/blog/${rp.slug}`}>
                        <Card className="overflow-hidden hover:shadow-lg transition-shadow cursor-pointer h-full flex flex-col">
                          {rp.featuredImage && (
                            <div className="h-40 overflow-hidden">
                              <img src={rp.featuredImage} alt={rpTitle} className="w-full h-full object-cover" />
                            </div>
                          )}
                          <div className="p-4 flex-1 flex flex-col">
                            <h3 className="font-semibold text-sm line-clamp-2 mb-2">{rpTitle}</h3>
                            {rpExcerpt && (
                              <p className="text-xs text-muted-foreground line-clamp-2">{rpExcerpt}</p>
                            )}
                          </div>
                        </Card>
                      </Link>
