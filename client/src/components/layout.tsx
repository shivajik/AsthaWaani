  ...
  return (
    ...
    >
      <div className="container mx-auto px-4 flex items-center justify-between">
        <Link href="/" className={cn("text-2xl font-serif font-bold tracking-tight hover:opacity-90 transition-opacity", isScrolled ? "text-primary" : "text-white")}>
            Asthawaani
        </Link>
        ...
        <nav className="hidden md:flex items-center gap-8">
          {navLinks.map((link) => (
            <Link 
              ...
              className={cn(
                ...
                location === link.href ? "text-primary font-bold" : (isScrolled ? "text-foreground" : "text-white/90")
              )}
            >
                {link.label}
            </Link>
          ...
          <Button 
            ...
            className={cn("gap-2", isScrolled ? "" : "text-white hover:text-white hover:bg-white/20")}
          >
        ...
        <Sheet>
          <SheetTrigger asChild>
            <Button variant="ghost" size="icon" className={cn("md:hidden", isScrolled ? "" : "text-white hover:bg-white/20")}>
    ...
    <footer className="bg-secondary text-secondary-foreground py-16">
      <div className="container mx-auto px-4">
        <div className="grid md:grid-cols-3 gap-12">
          <div>
            <h3 className="text-2xl font-serif font-bold mb-4 text-primary">Asthawaani</h3>
            ...
            <div className="flex gap-4">
              ...
              <div className="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center hover:bg-primary hover:text-primary-foreground transition-colors cursor-pointer">
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path fillRule="evenodd" d="M12.315 2c2.43 0 2.784.013 3.808.06 1.064.049 1.791.218 2.427.465a4.902 4.902 0 011.772 1.153 4.902 4.902 0 011.153 1.772c.247.636.416 1.363.465 2.427.048 1.067.06 1.407.06 4.123v.08c0 2.643-.012 2.987-.06 4.043-.049 1.064-.218 1.791-.465 2.427a4.902 4.902 0 01-1.153 1.772 4.902 4.902 0 01-1.772 1.153c-.636.247-1.363.416-2.427.465-1.067.047-1.407.06-4.123.06h-.08c-2.643 0-2.987-.012-4.043-.06-1.064-.049-1.791-.218-2.427-.465a4.902 4.902 0 01-1.772-1.153 4.902 4.902 0 01-1.153-1.772c-.247-.636-.416-1.363-.465-2.427-.047-1.067-.06-1.407-.06-4.123v-.08c0-2.643.013-2.987.06-4.043.049-1.064.218-1.791.465-2.427a4.902 4.902 0 011.153-1.772 4.902 4.902 0 011.772-1.153c.636-.247 1.363-.416 2.427-.465 1.067-.047 1.407-.06 4.123-.06h.08zm-4.125 0c-2.292 0-3.808.013-4.813.06-.99.049-1.639.218-2.127.465a2.902 2.902 0 00-1.153 1.153c-.247.488-.416 1.137-.465 2.127-.047 1.005-.06 2.52-.06 4.813v.066c0 2.292.013 3.808.06 4.813.049.99.218 1.639.465 2.127a2.902 2.902 0 001.153 1.153c.488.247 1.137.416 2.127.465 1.005.047 2.52.06 4.813.06h.066c2.292 0 3.808-.013 4.813-.06.99-.049 1.639-.218 2.127-.465a2.902 2.902 0 001.153-1.153c.247-.488.416-1.137.465-2.127.047-1.005.06-2.52.06-4.813v-.066c0-2.292-.013-3.808-.06-4.813-.049-.99-.218-1.639-.465-2.127a2.902 2.902 0 00-1.153-1.153c-.488-.247-1.137-.416-2.127-.465-1.005-.047-2.52-.06-4.813-.06h-.066z" clipRule="evenodd" /></svg>
              </div>
            </div>
          </div>
